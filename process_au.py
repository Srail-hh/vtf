...
for category_dir in os.listdir(root_dir):
    ...
    for object_dir in os.listdir(category_path):
        ...
        if not os.path.isdir(object_path):
            continue
        touch_results_dir = os.path.join(object_path, "haptic_poke_noise")
        ...
        for imag_id in range(len(touch_images)):
            touch_image_path = os.path.join(touch_results_dir, 'object_'+str(imag_id+1)+'.png')
# 根据触摸图像名获取对应的视觉图像名
            vision_image_path = os.path.join(vision_results_dir, 'object_{}.jpg'.format(imag_id+1))
