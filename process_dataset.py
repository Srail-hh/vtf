...
for category_dir in os.listdir(root_dir):
    ...
    if not os.path.isdir(category_path):
        continue
    for object_dir in os.listdir(category_path):
        object_path = os.path.join(category_path, object_dir)
        if not os.path.isdir(object_path):
            continue
        touch_results_dir = os.path.join(object_path, "tactile")
        vision_results_dir = os.path.join(object_path, "visual")
        ...
        for touch_image_name in touch_images:
            if 'z' in touch_image_name:
                # 找到'z'的位置，并移除'z'及其后的内容
                index = touch_image_name.index('z')
                vision_image_name = touch_image_name[:index] + '.jpg'
            elif 'x' in touch_image_name:
                index = touch_image_name.index('x')
                vision_image_name = touch_image_name[:index] + '.jpg'
            else:
                continue
            # 构建完整的路径
            touch_image_path = os.path.join(touch_results_dir, touch_image_name)
            vision_image_path = os.path.join(vision_results_dir, vision_image_name)
